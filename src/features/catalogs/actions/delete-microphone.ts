// src/features/catalogs/actions/delete-microphone.ts
...
export async function deleteMicrophone(
  microphoneId: string,
): Promise<ActionResult> {
  const activeUser = await requireSuperAdmin();
  if (!activeUser.ok) return activeUser;

  const parsed = catalogIdSchema.safeParse(microphoneId);
  ...
  await db.catalogMicrophone.delete({ where: { id: parsed.data } });

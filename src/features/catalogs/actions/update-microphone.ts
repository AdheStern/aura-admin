// src/features/catalogs/actions/update-microphone.ts
...
export async function updateMicrophone(
  microphoneId: string,
  brand: string,
  model: string,
  specJson: string,
  ...
  try {
    await db.catalogMicrophone.update({
  ...
  revalidatePath(`/catalogs/microphones/${parsed.data.equipmentId}`);
  return { ok: true };
}
